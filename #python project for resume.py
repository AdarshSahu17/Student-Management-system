...
class student:
    ...
    def __init__(self, root):
        ...
        Frem_title = Label(self.root, text="Student Management System", font=(
            "times new roman", "40", "bold"), bg="yellow", fg="red", bd=10, relief=SUNKEN).pack(side=TOP, fill=X)

        ###################### ---manneg fram---####################################
        M_fram = Frame(self.root, bd=4, relief=RIDGE, bg="crimson")
        M_fram.place(x=16, y=100, width=390, height=575)

        m_title = Label(M_fram, text="Manage Student", font=(
            "time new roman", "25", "bold"), bg="crimson", fg="white").grid(row=0, columnspan=2, pady=10)
        # m_title=Label(M_fram,text="Manege Student",font=("time new roman","25","bold"),bg="crimson",fg="white").pack(anchor=N)

        roll = Label(M_fram, text="Roll No.", font=("time new roman", "18", "bold"),
    ...
    def delete(self):
        if self.Roll_No_var.get() == "" or self.Name_var.get() == "" or self.Email_var.get() == "" or self.Gender_var.get() == "" or self.Contact_var.get() == "" or self.dob_var.get() == "":
            ...
            con = pymysql.connect(
                host="localhost", user="root", password="", database="std")
            cur = con.cursor()
            cur.execute("delete from Students where roll=%s",
                        self.Roll_No_var.get())
            con.commit()
            con.close()
            self.fetch_data()
    ...
    #     con = pymysql.connect(host="localhost", user="root", password="", database="str")
    #     cur = con.cursor()
    #
    #
    #     # cur.execute("select * from Students where "+str(self.search_by.get())+" LIKE '%"+str(self.search_text.get())+"%'")
    #     # cur.execute("select * from Students where " + str(self.search_by.get()) + "=" +str(self.search_text.get()) )
    #     # cur.execute("select * from Students where " + str(self.search_by.get()) + "="+str(self.search_text.get())+"%")
    #     cur.execute("select * from Students where Name = " +str(self.search_text.get()))
    #     rows=cur.fetchall()
    #
    #
    #     if len(rows)!=0:
    ...
    def search_data_roll_no(self):
        if self.Roll_No_var.get() == "":
            tmsg.showinfo("Error", " Enter the Roll_NO")
